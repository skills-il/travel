...
Single adult entry runs about 31 to 46 NIS at the main tourist sites (Ein Gedi and Banias 31, Masada 37, Caesarea 46), with children roughly half; smaller sites are lower. Tourist multi-site cards: Blue (3 sites) 90 NIS, Green (6 sites) 130 NIS, Orange (unlimited) 175 NIS. Each is valid for two weeks from the first visit, and none of them covers the Masada cable car.

Matmon is INPA's annual subscription and is the residents' option rather than a visitor product; for a short trip the tourist cards are the right comparison. Do not quote a Matmon price from memory: INPA does not publish the tier prices on its public price list, so send the user to parks.org.il for the current figure. One Matmon rule is worth knowing because it surprises people: a subscription covers entry everywhere EXCEPT the water sites (Chorshat Tal, Achziv, Gan HaShlosha/Sachne, Palmachim, and the sea turtle rescue centre visitor centre), where subscribers still pay 50%.
...
Masada cable car: a SEPARATE charge the Blue/Green/Orange tourist cards do NOT cover. It runs only inside the park's posted operating hours, which begin at 08:00, so it is NOT an option for a sunrise ascent; a sunrise visit goes up the Snake Path on foot, and the cable car is for the descent or for a later visit. Official Masada entrance is about 37 NIS adult (21 child, 19 senior); the cable car is billed separately and does NOT include entrance, at about 32 NIS one-way (16 child) or 54 NIS round-trip (32 child). Budget the cable car on top of entrance and any tourist card.
...
5. Tips: Bring water shoes and sunscreen SPF 50+; in the Dead Sea itself do not submerge your head or open your eyes, rinse in fresh water immediately after, and keep immersion short. For Masada, "arrive early" is not enough guidance: the cable car only runs from 08:00, so a sunrise ascent is on foot up the Snake Path, last entry to the park is an hour before posted closing, the last cable car up leaves an hour before closing, and in declared heat load the Snake Path has a morning last-ascent and last-descent cutoff. Those heat decisions are made that morning, so check the posted times on the day, not the night before. Masada also runs an advance visit-coordination booking that guarantees a slot
